...
module.exports = {
    ...
    execute: async (message , args , command, bot, Discord) => {
        if(message.author.id !== '429493473259814923') return message.channel.send('This command is in beta test and can only be used by EwasTaken')
        ...
        if (command === 'play'){
            ...
            if (ytdl.validateURL(args[1])) {
                ...
                const video = await video_finder(args.splice(1).join(' '));
                if (video){
